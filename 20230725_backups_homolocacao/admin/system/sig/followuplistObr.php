...
<div class="bg-info bottom20">
    <div class="col-md-12 bottom20 bg_orange">
        <p class="text-center"><i class="glyphicon glyphicon-check"></i> SIG's Cadastrados de Obras</p>
    </div>
    <div class="container-fluid">
        <table class="table table-bordered table-condensed">
            ...
            <?php
                ...
                $IdAssociado = filter_input(INPUT_GET, 'IdAssociado', FILTER_DEFAULT);
                $tipo        = filter_input(INPUT_GET, 'tipo', FILTER_DEFAULT);
                
                $readObr = $read;
                $readObr->ExeRead("tb_obras_obr", "WHERE Codigo = {$Codigo}");

                if(!$readObr->getResult()):
                    WSErro("Não Existe OBRA cadastrada com esse Código", WS_INFOR);
                    else:
                    foreach($readObr->getResult() as $obr):
                        extract($obr);      
            ?>
            <tr>
                <td colspan="5"><i class="glyphicon glyphicon-check"></i> <b><?= $Projeto;?></b></td>
            </tr>
            <tr>
                <th colspan="4"></th>
                <th><b>Novo SIG</b></th>
            ...
            <?php
                ...
                if ($del):
                    require ('_models/AdminFollowupObra.class.php');
                    $delete = new AdminFollowupObra;
                    $delete->ExeDelete($del);
                    header("Location: painel.php?exe=sig/followuplistObr&Codigo={$Cod}");
                endif;
            ...
            <tr>
                ...
                <td><a href="painel.php?exe=sig/followuplistObr&del=<?= $IDFOLLOWUP;?>&Cod=<?= $Codigo;?>" title="deletar"><i class="glyphicon glyphicon-remove"></i></a></td>
            </tr>
            <?php
                endforeach;
                endif;
        ...
        <div class="modal-footer">
            <div class="col-md-12">
                <a href="painel.php?exe=sig/obra" class="btn btn-primary" title="Nova Pesquisa"><i class="glyphicon glyphicon-ok"></i> Nova Pesquisa</a>
            </div>
        </div>
    </div>
                ...
                <div class="modal-body">
                    <?php
                        ...
                        if ($data && isset($data['Send'])):
                            ...
                            unset($data['Send']);
                            require('_models/AdminFollowupObra.class.php');
                            $cadastra = new AdminFollowupObra;
                    ...
                    <form class="search" name="PostForm" action="" method="post">
                        ...
                        <input type="hidden" name="INDTIPO" id="tipo" value="<?= $tipo;?>" />
        
                        <div class="col-lg-12">
                            <p><b>OBRA:</b> <span id="nomeObra"></span></p>
...
<script>
    $('#followup').on('show.bs.modal', function (event) {
      ...
      var modal = $(this)
      modal.find('.modal-title').text('SIG Obra - ' + obra)
      //modal.find('.modal-body input').val(id)
      modal.find('#id').val(id)
      modal.find('#associado').val(associado)
      modal.find('#obra').val(obra)
      modal.find('#nomeObra').text(obra)
